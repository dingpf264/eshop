		...
		<div class="span18 last">
			<form id="productForm"
				...
				<div id="result" class="result table clearfix">
					<ul>
						<s:set value="#request.pageProducts2.list" id="ps"></s:set>
						<s:if test="#ps != null">
				...
				<div class="pagination">
					<s:set value="#request.pageProducts2" id="pageProducts"></s:set>
						<!-- 根据二级分类id获取该页的所有商品记录 -->
						<s:if test="csid != null">
							<s:if test="#pageProducts != null">
								...
								<s:if test="#pageProducts.currentPage != 1">
									<a href="${ pageContext.request.contextPath }/product_findPageProductsByCategorysecond.action?cid=<s:property value="cid"/>&currentPage=1" class="firstPage">&nbsp;</a>
									<a href="${ pageContext.request.contextPath }/product_findPageProductsByCategorysecond.action?cid=<s:property value="cid"/>&currentPage=<s:property value="#pageProducts.currentPage-1"/>" class="previousPage">&nbsp;</a>
								</s:if>
								
								<s:iterator var="i" begin="1" end="#pageProducts.totalPage">
									<s:if test="#pageProducts.currentPage != #i">
										<a href="${ pageContext.request.contextPath }/product_findPageProductsByCategorysecond.action?cid=<s:property value="cid"/>&currentPage=<s:property value="#i"/>"><s:property value="#i"/></a>
									</s:if>
									<s:else>
								...
								<s:if test="#pageProducts.currentPage != #pageProducts.totalPage">	
									<a class="nextPage" href="${ pageContext.request.contextPath }/product_findPageProductsByCategorysecond.action?cid=<s:property value="cid"/>&currentPage=<s:property value="%{#pageProducts.currentPage}+1"/>">&nbsp;</a>
									<a class="lastPage" href="${ pageContext.request.contextPath }/product_findPageProductsByCategorysecond.action?cid=<s:property value="cid"/>&currentPage=<s:property value="%{#pageProducts.totalPage}"/>">&nbsp;</a>
								</s:if>
							</s:if>
							<s:else>
								<center>
									<h3>该分类的商品正在仅仅备货中.....请君耐心等待\(^o^)/~</h3>
								</center>
							</s:else>
					</s:if>	
				</div>
